...
namespace Assets.Elenesski.Camera.Utilities {
    ...
    public class GenericMoveCamera : MonoBehaviour {
        ...
        public void Start() {
            ...
            _RotateY = new Movement(aAmount =>
            {
                var rotationAmount = gameObject.transform.rotation.eulerAngles + Vector3.left*aAmount;
                // hack to prevent gimbal lock
                if (rotationAmount.x < 85 || 275 < rotationAmount.x)
                {
                    gameObject.transform.Rotate(Vector3.left * aAmount);
                }
            }, () => RotateDampenRate);
